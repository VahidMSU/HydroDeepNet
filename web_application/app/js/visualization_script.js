...
async function populateDropdowns() {
  ...
  try {
    ...
    if (!data.names || !data.variables) {
      throw new Error(
        "Invalid response structure. Expected 'names' and 'variables'."
      );
    }
...
async function updateVisualizations() {
  ...
  const variables = Array.from(
    document.getElementById("variable").selectedOptions
  ).map((opt) => opt.value);
  ...
  try {
    ...
    toggleError(
      "An error occurred while fetching visualizations. Please try again."
    );
  } finally {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const showVisualizationsButton = document.getElementById(
    "show_visualizations"
  );
