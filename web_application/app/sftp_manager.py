...
def create_sftp_user(username, password=None):
    ...
    try:
        ...
        # Log success
        logger.info(f"SFTP user {username} created successfully")
        return {"success": True, "message": f"SFTP user {username} created successfully with Apache access. Previously exists? {previously_existed}"}
    except subprocess.CalledProcessError as e:
        logger.error(f"Failed to create SFTP user {username}: {e}")
        return {"success": False, "message": f"Failed to create SFTP user {username}: {e}"}
...
def delete_sftp_user(username):
    ...
    try:
        ...
        # Log success
        logger.info(f"SFTP user {username} deleted successfully")
        return {"success": True, "message": f"SFTP user {username} deleted successfully."}
    
    except subprocess.CalledProcessError as e:
        logger.error(f"Failed to delete SFTP user {username}: {e}")
        return {"success": False, "message": f"Failed to delete SFTP user {username}: {e}"}
