    ...
    <Box display="flex" flexDirection="column" p={3}>
      <Box sx={{ p: 3, textAlign: "center" }}>
        ...
        <Typography variant="h7" sx={{ color: 'white' }}>
          HydroGeoDataset provides high-resolution hydrological, environmental, and climate data.
          It includes datasets such as PRISM, LOCA2, and Wellogic records.
        </Typography>
      </Box>

      <Box mt={3} display="flex" gap={3}>
        <Card sx={{ width: '40%', bgcolor: "white" }}>
          <CardContent>
            <Typography variant="h5" align='center' sx={{ fontWeight: 'bold', color: '#2b2b2c', mb: 2 }}>Data Query</Typography>
            <HydroGeoDatasetForm
              formData={formData}
      ...
      {data && (
        <Card sx={{ mt: 3, p: 2 }}>
          ...
          <ChatInterface dataResults={data.data} />
        </Card>
      )}
    </Box>
  );
...
//       const result = await response.json();
//       setData(result); // This will trigger the ChatInterface to analyze the new data
//     } catch (error) {
//       console.error('Error fetching data:', error);
//     }
//   };
...
//         {/* Display Data and Chat Interface */}
//         {data && (
//           <DataDisplay>
//             <Title>Results</Title>
//             <DataResults>
