...
def check_gwflow_integration(base_directory, level, model_name):
    names = os.listdir(os.path.join(base_directory, f"SWATplus_by_VPUID/{level}"))
    ...
    for name in names:
        model_base = os.path.join(base_directory, f'SWATplus_by_VPUID/{level}/{name}/{model_name}')
            
        gwflow_path = os.path.join(model_base, 'Scenarios/Default/TxtInOut/gwflow_flux_recharge')
        modflow_path = os.path.join(base_directory, f'SWATplus_by_VPUID/{level}/{name}/MODFLOW_250/rasters_input')
...
def process_gwflow(LEVEL, name,start_year, end_year,MODEL_NAME):

    RESOLUTION = 250
    BASE_PATH = "/data/MyDataBase/SWATGenXAppData/"

    return creating_gwflow(BASE_PATH, LEVEL, name, RESOLUTION,start_year, end_year,MODEL_NAME, CRS = 26990)


    
def creating_gwflow(BASE_PATH,LEVEL, NAME, RESOLUTION,start_year, end_year,MODEL_NAME, CRS = 26990):
    print('begin')
    

    inout_path = Setting_env_and_returning_object_path (BASE_PATH, NAME, LEVEL, RESOLUTION,start_year, end_year, MODEL_NAME)
    ...
    #####  extracting fortmatted channel-cell, hru-cell and cell-hru tables from the above objects
...
def getting_grid_table(outpath_object, gis_folder, gwflow_target_path,feet_to_meter=0.3048):
    ...
    Thick ['Thick'] =np.where( Thick ['Thick'] <5, 5, Thick ['Thick'] )
...
def setting_env(BASE_PATH, LEVEL, NAME, start_year, end_year, MODEL_NAME):
    ...
    gwflow_target_path = os.path.join(BASE_PATH, f"SWATplus_by_VPUID/{LEVEL}/{NAME}/{MODEL_NAME}/Scenarios/Default/TxtInOut")
    gwflow_input_target_path = os.path.join(BASE_PATH, f"SWATplus_by_VPUID/{LEVEL}/{NAME}/{MODEL_NAME}")
    ...
    shutil.copytree (fr"/data/MyDataBase/SWATGenXAppData/SWATplus_by_VPUID/{LEVEL}/{NAME}/SWAT_MODEL/Scenarios/Default/TxtInOut", gwflow_target_path,dirs_exist_ok=True)
...
def process_path_objects(path_objects, grids_point_path, gis_folder, grids_polygon):
    ...
    for item in path_objects:
        ...
        if Type == 'raster':
            ...
            object = object.set_geometry('geometry')
            object = object.set_crs('EPSG:26990')
...
def Setting_env_and_returning_object_path(BASE_PATH, NAME, LEVEL, RESOLUTION,start_year, end_year, MODEL_NAME):

    ## setting the environment
    gwflow_data, gwflow_target_path, gis_folder, rech_out_folder = setting_env(BASE_PATH, LEVEL, NAME, start_year, end_year, MODEL_NAME)
    arcpy.env.overwriteOutput = True
    #### Modflow and groundwater data    
    Modflow_directory    = fr"/data/MyDataBase/SWATGenXAppData/SWATplus_by_VPUID/{LEVEL}/{NAME}/MODFLOW_{RESOLUTION}/"                                ## directory
    ...
    SWAT_model_directory = fr"/data/MyDataBase/SWATGenXAppData/SWATplus_by_VPUID/{LEVEL}/{NAME}/"
    SWAT_shape_dire      = os.path.join(SWAT_model_directory, "SWAT_MODEL/Watershed/Shapes/") 
    ...
    inout_path = {'outpath_object':outpath_object, 
...
#    if completion_status == 'done':  
    try:
        ...
        completion_status = f'model execution finished'
    except Exception as e:    
...
def getting_CELL_CHANNEL_HRU_TABLES(outpath_object, RESOLUTION,gwflow_target_path):
    ...
    HRUs['HRUs_Area'] = HRUs['HRUs_Area'].astype(float).round(2)
    HRUs['Cell_Area'] = HRUs['Cell_Area'].astype(int)
    HRUs['poly_Area'] = HRUs['poly_Area'].astype(float).round(2)
...
def writing_SWATplus_GW_input(BASE_PATH, LEVEL, NAME, RESOLUTION, CEll_HRU_TABLE, HRU_CELL_TABLE, CELL_CHANNEL_TABLE, GRIDS_TABLE, num_rows, num_cols, Zone, ZONE_K_TABLE, grids_point, gwflow_target_path, gis_folder):
    
    shutil.copy2    (fr"/data/MyDataBase/SWATGenXAppData/bin/swatplus.exe", gis_folder)
    # Reading the file
    rout_unit = pd.read_csv(
        os.path.join(f"/data/MyDataBase/SWATGenXAppData/SWATplus_by_VPUID/{LEVEL}/{NAME}/SWAT_MODEL/Scenarios/Default/TxtInOut/rout_unit.con"),
        skiprows=2,
        sep='\s+',
    ...
    with open(os.path.join(gwflow_target_path, r"rout_unit.con"), 'w') as file:
        ...
        max_widths = [max([len(str(x)) for x in rout_unit[col]]) for col in rout_unit.columns]
    ...
    object_cnt = pd.read_csv(os.path.join(gwflow_target_path, f"object.cnt"), skiprows=1, sep='\s+')   # Reading the file
    ...
    with open(fr'/data/MyDataBase/SWATGenXAppData/SWATplus_by_VPUID/{LEVEL}/{NAME}/SWAT_MODEL/Scenarios/Default/TxtInOut/file.cio','r') as file:
...
def create_recharge_shapefile(LEVEL, NAME,RESOLUTION, recharge, year,rech_out_folder):
    input_grids_path = f'/data/MyDataBase/SWATGenXAppData/SWATplus_by_VPUID/{LEVEL}/{NAME}/MODFLOW_{RESOLUTION}/Grids_MODFLOW/Grids_MODFLOW.shp'
    output_recharge_path = os.path.join(rech_out_folder, f'recharge_{year}.shp')
    SWAT_dem_path = f'/data/MyDataBase/SWATGenXAppData/SWATplus_by_VPUID/{LEVEL}/{NAME}/DEM_{RESOLUTION}m.tif'
