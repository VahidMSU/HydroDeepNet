...
def import_ml_database(RESOLUTION):
    base_data_path    = f"/data/MyDataBase/SWATGenXAppData/observations/rasters_{RESOLUTION}m_with_observations.pk1"
    NHDPlusVAA_path =   "/data/MyDataBase/SWATGenXAppData/NHDPlusData/NHDPlusFlowlineVAA.pkl"
    NHDWaterbody_path = "/data/MyDataBase/SWATGenXAppData/NHDPlusData/NHDWaterbody.pkl"
    NHDFlowlines_path = "/data/MyDataBase/SWATGenXAppData/NHDPlusData/NHDFlowline.pkl"
    gSSURGO_path      = "/data/MyDataBase/SWATGenXAppData/Soil/SWAT_gSSURGO.csv"
    ...
    gSSURGO               = pd.read_csv(gSSURGO_path)[["muid", "hyd_grp", "texture"]]
    ...
    flowlines_waterbodies_VAA_base_data = pd.merge(base_data, flowlines_waterbodies_VAA, on = 'NHDPlusID', how='right').rename(columns = {'index_left':'observed_well_data'})
...
mldata, all_data = import_ml_database(RESOLUTION = 250)
...
models= [
        (XGBRFRegressor(random_state=42, tree_method='auto'), 
...
for model, param_grid, model_name, search_strategy in models:
    ...
    model_path = f'/data/MyDataBase/SWATGenXAppData/codes/gw_machine_learning/{model_name}.joblib'
...
# now fit the best model to the entire data
# first load the best model
# best_model = joblib.load(model_path)
# all_data = all_data[num_features+rbf_features+categorical_features+target_variable]
# all_data = all_data.fillna(-999)
# X = all_data.drop(columns=target_variable)
# y = all_data[target_variable].values.ravel()
# best_model.fit(X, y)