...
path = "/data/MyDataBase/SWATGenXAppData/SWAT_input/huc12"
NAMES = os.listdir(path)

# Ensure the output directory exists
output_dir = "/data/MyDataBase/SWATGenXAppData/codes/SWAT_recharge/annual"
...
for year in range(2004, 2021):
    ...
    for NAME in NAMES:
        VPUID = f"0{NAME[:3]}"
        verifications = f"E:/MyDataBase/SWATplus_by_VPUID/{VPUID}/huc12/{NAME}/recharg_output_SWAT_gwflow_MODEL/verification_stage_{ver_num}"
    ...
    if len(rasters_to_add) > 1:  # Ensures we have rasters to add
        ...
        arcpy.MosaicToNewRaster_management(rasters_to_add, output_dir, os.path.basename(output_path),
                                           pixel_type="32_BIT_FLOAT", number_of_bands=1, mosaic_method="SUM")
