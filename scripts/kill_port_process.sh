...
# Usage: ./kill_port_process.sh [port_number]

PORT=${1:-5050}  # Default to port 5050 if not specified
...
for PID in $UNIQUE_PROCESSES; do
  ...
  if kill -15 $PID 2>/dev/null; then
    ...
    # Wait a moment to see if it stops
    sleep 2
  else
...
if lsof -i :$PORT > /dev/null 2>&1 || fuser $PORT/tcp > /dev/null 2>&1; then
  ...
  log "All processes have been terminated gracefully"
fi
