...
chmod +x ${SCRIPTS_DIR}/fix_redis_wrongtype.py
chmod +x ${SCRIPTS_DIR}/cleanup_corrupted_tasks.py
chmod +x ${SCRIPTS_DIR}/cleanup_corrupted_tasks_wrapper.sh
chmod +x ${SCRIPTS_DIR}/restart_corrupted_celery.sh
chmod +x ${SCRIPTS_DIR}/monitor_celery_status.py

echo "Creating symbolic links in /usr/local/bin for easy access..."

# Create symbolic links in /usr/local/bin if running as root
if [[ $EUID -eq 0 ]]; then
    ln -sf ${SCRIPTS_DIR}/cleanup_corrupted_tasks_wrapper.sh /usr/local/bin/cleanup-celery-tasks
    ln -sf ${SCRIPTS_DIR}/restart_corrupted_celery.sh /usr/local/bin/restart-celery
