...
def training_loop(epochs, model, optimizer, scheduler, loss_fn, x_dict, edge_index_dict, edge_attr_dict, data, grid_centroids_df, logger, var_to_predict, patience=10, accumulation_steps=2):
    ...
    for epoch in range(epochs):
        ...
        # Split data into smaller batches if needed (gradient accumulation)
        accumulation_counter = 0
        running_loss = 0.0

        for batch_step in range(accumulation_steps):
            ...
            # Step optimizer and clear gradients after accumulation_steps batches
            ...
            # Empty unused cache during training
        ...
        print(f"Epoch {epoch}, Training Loss: {running_loss:.4f}, Validation Loss: {val_loss.item():.4f}")
