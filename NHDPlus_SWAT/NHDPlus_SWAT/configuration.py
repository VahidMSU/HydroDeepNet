...
def check_configuration(VPUID,landuse_epoch) -> str:
    ...
    streams_path = os.path.join(f'/data/MyDataBase/SWATGenXAppData/NHDPlusData/SWATPlus_NHDPlus/{VPUID}/streams.pkl')
    original_resolution = 30
    
    base_input_raster = f'/data/MyDataBase/SWATGenXAppData/DEM/VPUID/{VPUID}/'
    
    streamflow_meta_data_directory = f"/data/MyDataBase/SWATGenXAppData/streamflow_stations/VPUID/{VPUID}/meta_{VPUID}.csv"
    
    streamflow_stations_directory = f"/data/MyDataBase/SWATGenXAppData/USGS/streamflow_stations/VPUID/{VPUID}/streamflow_stations_{VPUID}.shp"
    
    input_raster = os.path.join(base_input_raster, "USGS_DEM_30m.tif")
    landuse_raster = f"/data/MyDataBase/SWATGenXAppData/LandUse/NLCD_CONUS/{VPUID}/NLCD_{VPUID}_{landuse_epoch}_250m.tif"
    soil_raster = f"/data/MyDataBase/SWATGenXAppData/Soil/gSSURGO_CONUS/{VPUID}/soil_{VPUID}.tif"
    ...
    SWAT_MODEL_PRISM_path = f'/data/MyDataBase/SWATGenXAppData/PRISM/VPUID/{VPUID}/PRISM_grid.shp'
