...
class runHUC():
    ...
    def runProject(self, dataDir, scale, minHRUha):
        ...
        self.LanduseTable = '/data/MyDataBase/SWATGenXAppData/LandUse/landuse_lookup.csv'
...
def main(VPUID,LEVEL,NAME, MODEL_NAME):
    ...
    direc = f"/data/MyDataBase/SWATGenXAppData/SWATplus_by_VPUID/{VPUID}/{LEVEL}/{NAME}/{MODEL_NAME}/{MODEL_NAME}.qgs"
    ## delete the database file if it exists
    if os.path.exists(f"/data/MyDataBase/SWATGenXAppData/SWATplus_by_VPUID/{VPUID}/{LEVEL}/{NAME}/{MODEL_NAME}/"):
        ### remove all files and no directories
        files = glob.glob(f"/data/MyDataBase/SWATGenXAppData/SWATplus_by_VPUID/{VPUID}/{LEVEL}/{NAME}/{MODEL_NAME}/*")
        for f in files:
            ## do not remove directories
            if os.path.isfile(f):
                os.remove(f)
    else:
        os.makedirs(f"/data/MyDataBase/SWATGenXAppData/SWATplus_by_VPUID/{VPUID}/{LEVEL}/{NAME}/{MODEL_NAME}/")
    dataDir = "H:/Data"
    scale = 8
    minHRUha = 0.00
    inletId = 0
