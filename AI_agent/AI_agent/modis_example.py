...
try:
    ...
    from AI_agent.AI_agent.modis_report import (
    ...
    try:
        ...
        from AI_agent.modis_report import (
            generate_modis_report, 
            generate_comprehensive_modis_report,
            analyze_modis_environmental_indicators,
            create_integrated_landcover_report,
...
def load_sample_climate_data(region_name=None, start_year=2000, end_year=2020):
    ...
    """
    # Generate dates (monthly for simplicity)
    ...
    season = -np.cos(2 * np.pi * (month_idx - 1) / 12)  # Peaks in summer months
    ...
    base_precip = 50  # Base monthly precipitation
    precip_seasonal = 30 * (np.sin(2 * np.pi * (month_idx - 3) / 12) + 0.5)  # Peak in spring
