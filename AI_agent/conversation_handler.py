...
class ConversationalAgent:
    ...
    def retrieve_data(self, query_info):
        """Retrieve the necessary data based on query info."""
        ...
        all_cached = True
        for year in years:
            year_data = self.context.get_cached_data(county, state, year)
            if year_data:
                cached_data[year] = year_data
            else:
                all_cached = False
        ...
        # If not cached, retrieve the data
        ...
        try:
            ...
            # Only get climate data if needed
            if analysis_type in ['climate', 'both']:
                try:
                    data_result['climate'] = PRISM_Dataset(config).get_spatial_average_over_time()
                except Exception as e:
                    print(f"Error retrieving climate data: {e}")
            
            # Try to get landcover data with fallbacks for years
            if analysis_type in ['crop', 'both']:
                try:
                    # Try to get the requested years
                    data_result['landcover'] = cdl_trends(config)
                    
                    # If no data returned, try to get data for nearby years
                    if not data_result['landcover']:
                        print(f"No data for {years}. Trying nearby years...")
                        
                        # Try years one at a time from a wider range
                        available_data = {}
                        for year in range(min(years) - 2, max(years) + 3):
                            try:
                                alt_config = config.copy()
                                alt_config['start_year'] = year
                                alt_config['end_year'] = year
                                year_data = cdl_trends(alt_config)
                                if year_data:
                                    available_data.update(year_data)
                                    print(f"Found data for alternate year: {year}")
                            except Exception as alt_e:
                                print(f"Error retrieving alternate year {year}: {alt_e}")
                        
                        if available_data:
                            data_result['landcover'] = available_data
                            print(f"Using alternative years: {list(available_data.keys())}")
                except Exception as e:
                    print(f"Error retrieving landcover data: {e}")
                
            # Cache the result for future use if we got any data
            if data_result['landcover'] or data_result['climate']:
                for year in years:
                    self.context.cache_data(county, state, year, data_result)
