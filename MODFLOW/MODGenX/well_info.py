...
def well_data_import(mf,top, load_raster_args, z_botm, active, grids_path, MODEL_NAME, gpm_to_cmd=5.678, fitToMeter=0.3048):
	...
	obs = pd.read_pickle("/data/MyDataBase/SWATGenXAppData/observations/observations_original.pk1")
	obs = obs.drop(columns=['row','col'])
	grids = pd.read_pickle(fr"/data/MyDataBase/SWATGenXAppData/SWAT_input/{LEVEL}/{NAME}/{MODEL_NAME}/Grids_MODFLOW.pk1")
	grids ['row']= grids['Row']
	grids ['col'] = grids['Col']
