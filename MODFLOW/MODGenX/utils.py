...
def defining_bound_and_active(BASE_PATH, subbasin_path, raster_folder, RESOLUTION, SWAT_dem_path):
    ...
    arcpy.env.extent = SWAT_dem_path
...
def load_raster(path, load_raster_args, BASE_PATH='/data/MyDataBase/SWATGenXAppData/'):
...
def clip_raster_by_another(BASE_PATH, raster_path, in_masking, output_path):
    ...
    # Get the extent and cell size of the mask raster
    mask_raster = arcpy.Raster(in_masking)
    extent = mask_raster.extent
    cell_size = mask_raster.meanCellWidth  # Assuming square cells, so just one dimension is needed

    # Get number of rows and columns for in_masking raster
    mask_n_rows = int((extent.YMax - extent.YMin) // cell_size)
    mask_n_cols = int((extent.XMax - extent.XMin) // cell_size)

    # Create a temporary resampled raster with the same cell size, rows, and columns as in_masking
    ### generate a random name for the temporary raster
    import uuid
...
def create_shapefile_from_modflow_grid_arcpy(BASE_PATH, model_path, MODEL_NAME, out_shp, raster_path):
    ...
    x_min_raster = raster_desc.extent.XMin
    y_max_raster = raster_desc.extent.YMax
