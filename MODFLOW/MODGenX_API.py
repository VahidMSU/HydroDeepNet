...
if __name__ == '__main__':
	...
	NAMES = os.listdir(fr'/data/MyDataBase/SWATGenXAppData/SWAT_input/{LEVEL}/')
	...
	if test:
		NAME = '04166000'  ### testing, keep it or condition it
		MODEL_NAME = 'MODFLOW_30m'
			...
			for MODEL_NAME in modelnames:
				...
				for NAME in NAMES:
					...
					if os.path.exists(path_to_metrics):
						...
						if not os.path.exists(path_to_metrics) and os.path.exists(f"/data/MyDataBase/SWATGenXAppData/SWAT_input/{LEVEL}/{NAME}/{MODEL_NAME}/Grids_MODFLOW.pk1"):
							print(f"{NAME} failed probably due to lack of good data")

							continue
						continue

					subbasin_path = f"/data/MyDataBase/SWATGenXAppData/SWAT_input/huc12/{NAME}/{SWAT_MODEL_NAME}/Watershed/Shapes/SWAT_plus_subbasins.shp"
					...
					if not (extent[0] < centroid.x < extent[2] and extent[1] < centroid.y < extent[3]):
						...
						second_path = f"/data/MyDataBase/SWATGenXAppData/SWAT_input/{LEVEL}/{NAME}/"
