...
def check_source_target_existance(VPUID, NAME, MODEL_NAME):
	...
	### the MODEL_NAME does not exists in the source, return False
	if not os.path.exists(os.path.join(source_path, MODEL_NAME)):
		return False
...
if __name__ == "__main__":
		...
		while True:
				...
				for NAME in NAMES:
						print(f"{NAME} exists {MODEL_NAME}")
						if len(NAME) > 10:
								continue
						...
						wrapped_model_evaluator = get_wrapped_model_evaluator(NAME, MODEL_NAME, VPUID, BASE_PATH, False, True, False)
