...
def remove_streamflow_data():
    target_path = "/data/MyDataBase/SWATplus_by_VPUID/0000/huc12/"
    NAMES = os.listdir(target_path)
    NAMES.remove("log.txt")
    for NAME in NAMES:

        source_path = f"/data2/MyDataBase/SWATplus_by_VPUID/0000/huc12/{NAME}/streamflow_data"
        target_path = f"/data/MyDataBase/SWATplus_by_VPUID/0000/huc12/{NAME}/streamflow_data"
...
def correct_streamflows():
    ...
    for NAME in NAMES:
        ...
        for file in files:
            ...
            plt.xlabel("Date")
            plt.ylabel(f"cumulative streamflow (cfs)")
