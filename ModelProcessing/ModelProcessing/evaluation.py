...
from ModelProcessing.visualization import plot_streamflow, plot_streamflow_monthly

try:
    from ModelProcessing.utils import write_model_parameters
    from ModelProcessing.utils import filling_observations
except Exception:
    from utils import  filling_observations, write_model_parameters
...
class SwatModelEvaluator:
    ...
    def cal_streamflow_obj_val(self, file, channel_sd_day_data):
        ...
        try:
            ...
            simulated_monthly = simulated_aligned_indexed.resample('ME').sum()
    ...
    def read_observed_data(self, file):
        ...
        try:
            ...
            observed_monthly = observed_monthly.resample('ME').sum()
    ...
    def plot_streamflow(self, observed, simulated, title, time_step, name_score):
        ...
        try:
            # Use the centralized plotting function from visualization.py
            plot_streamflow(
                observed=observed,
                simulated=simulated,
                title=title,
                ...
                nyskip=self.nyskip,
                fig_files_paths=self.fig_files_paths,
                stage=self.stage,
                model_log_path=self.model_log_path,
                cms_to_cfs=self.cms_to_cfs
            )
    ...
    def plot_streamflow_monthly(self, observed_monthly, simulated_monthly, title, time_step, name_score):
        ...
        try:
            # Use the centralized plotting function from visualization.py
            plot_streamflow_monthly(
                observed_monthly=observed_monthly,
                simulated_monthly=simulated_monthly,
                title=title,
                time_step=time_step,
                name_score=name_score,
                fig_files_paths=self.fig_files_paths,
