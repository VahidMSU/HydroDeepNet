...
logger = get_logger('ModelProcessing.utils')
...
def write_model_parameters(param_files, params, problem, operation_types, scenario_TxtInOut, original_TxtInOut):
    logger.info(f"Writing model parameters to {scenario_TxtInOut}")
...
def filling_observations(df_complete, observed):
    ...
    """
    assert 'date' in df_complete.columns, "The 'date' column is missing in df_complete"
    assert 'date' in observed.columns, "The 'date' column is missing in observed"
    assert 'streamflow' in observed.columns, "The 'streamflow' column is missing in observed"
    ### assert no nan
    ...
    ## assert no nan
    assert observed_with_gaps['streamflow'].isnull().sum() == 0, "Nan values in the streamflow column"
...
def is_cpu_usage_low(intervals=2, n_processes=210, swat_model=True, process_name='swatplus'):
    """Check if the current CPU usage is lower than the threshold."""
    CPU_USAGE_AVERAGE = psutil.cpu_percent(interval = intervals)
    logger.info(f"CPU Usage: {CPU_USAGE_AVERAGE}%")
    used_memory = ram_usage()
    logger.info(f"RAM Usage: {used_memory:.2f} GB")
    if swat_model:
        swatplus_process_count = count_processes(process_name = 'swatplus')
        logger.info(f"Number of SWAT+ processes: {swatplus_process_count}")
        #time.sleep(15)
...
def removing_directory(path):
    if os.path.exists(path):
        try:
            ...
            logger.info(f"Error removing folder: {e}")

def copy_files(NAME, VPUID, MODEL_NAME, path_to_copy, OUTPUT_PATH):
    logger.info(f"copy_files: {VPUID}, {NAME}, {MODEL_NAME}")
    try:
        if not os.path.exists(os.path.join(OUTPUT_PATH, VPUID, 'huc12', NAME)):
            ...
            logger.info(f'created path: {os.path.join(OUTPUT_PATH, VPUID, "huc12", NAME)}')
        ...
        if not os.path.exists(os.path.join(OUTPUT_PATH, VPUID, 'huc12', NAME, MODEL_NAME, 'streamflow_data')):
            ...
            logger.info(f'creating path: {os.path.join(OUTPUT_PATH, VPUID, "huc12", NAME, MODEL_NAME, "streamflow_data")}')
        ...
        if not os.path.exists(os.path.join(OUTPUT_PATH, VPUID, 'huc12', NAME, MODEL_NAME)):
            ...
            logger.info(f'created path: {os.path.join(OUTPUT_PATH, VPUID, "huc12", NAME, MODEL_NAME)}')
        ...
        logger.info(f'target path: {os.path.join(OUTPUT_PATH, VPUID, "huc12", NAME, MODEL_NAME)}')

    except PermissionError as e:
        logger.info(f'Permission denied: {e}')
    logger.info(f'path: {os.path.join(OUTPUT_PATH, VPUID, "huc12", NAME, MODEL_NAME)}')
    return False
            ...
            if folder_name not in ['Default'] or 'verification' in folder_name:
                ...
                if os.path.isdir(folder_path):
                    try:
                        ...
                        logger.info(f'Not deleted: {folder_path}, due to error: {e}')
...
def activate_ET_print(TxtInOut_path):
    with open(os.path.join(TxtInOut_path,'print.prt'), 'r') as file:
        lines=file.readlines()
        for i in range(len(lines)):
            if 'lsunit_wb' in lines[i]:
                lines[i]='lsunit_wb                       n             y             y             y  \n'
