...
def start_extracting(BASE_PATH, list_of_HUC,  LEVEL, VPUID):
    ...
    print(f"list of huc12 requested: {list_of_HUC}")
    print(f"stream['LEVEL']:{LEVEL}")
    streams = streams[streams["huc12"].isin(list_of_HUC)].reset_index(drop=True)
    ...
    def identify_lake_out(streams):
        ...
        for lake_id, group in grouped_streams:
            ...
            if len(group) == 1:
                ...
                # Check if dn_row is not empty and LakeId is -9999
                if not dn_row.empty and dn_row['LakeId'].iloc[0] == -9999:
                    ...
                    # Assign -9999 to LakeWithin for the matching HydroSeq
