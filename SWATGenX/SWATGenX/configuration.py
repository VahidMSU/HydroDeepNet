...
def check_configuration(VPUID, landuse_epoch) -> str:
    ...
    landuse_raster = f"{NLCD_path}/VPUID/{VPUID}/NLCD_{VPUID}_{landuse_epoch}_250m.tif"
    soil_raster = f"{gSSUGO_path}/VPUID/{VPUID}/soil_{VPUID}.tif"
    ...
    assert os.path.exists(landuse_raster), "Landuse data is not built. Please run NLCD_extraction.py"
    assert os.path.exists(soil_raster), "Soil data is not built. Please run Soil_extraction.py"
    assert os.path.exists(input_raster), "DEM data is not built. Please run DEM_extraction.py"
