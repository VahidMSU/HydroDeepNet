...
def wrapped_build_geospatial_infrastructure(VPUID, landuse_epoch):
    ...
    NHDPlus_extract_by_VPUID(VPUID)  ## does not require arcpy anymore
    NHDPlus_preprocessing(VPUID)  ## does not require arcpy anymore
    DEM_extract_by_VPUID(VPUID)  ### does not require arcpy anymore
