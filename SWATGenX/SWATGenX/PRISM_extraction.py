...
def clip_PRISM_by_VPUID(VPUID):
    ...
    output_path = f"{SWATGenXPaths.PRISM_path}/VPUID/{VPUID}/PRISM_grid.shp"
    SWAT_MODEL_PRISM_path = f'{SWATGenXPaths.PRISM_path}/VPUID/{VPUID}/'
...
def _extracted_from_clip_PRISM_by_VPUID_7(VPUID, output_path):
    ...
    WBDHU8 = pd.read_pickle(f"{SWATGenXPaths.extracted_nhd_swatplus_path}/{VPUID}/WBDHU8.pkl").to_crs("EPSG:4326")
    extent = WBDHU8.total_bounds
    PRISM = gpd.read_file(SWATGenXPaths.PRISM_mesh_path).to_crs("EPSG:4326")
    clipped_PRISM_HUC12 = PRISM.cx[extent[0]:extent[2], extent[1]:extent[3]]
    os.makedirs(f"{SWATGenXPaths.PRISM_path}/VPUID/{VPUID}", exist_ok=True)
    WBDHU12 = pd.read_pickle(f"{SWATGenXPaths.extracted_nhd_swatplus_path}/{VPUID}/WBDHU12.pkl").to_crs("EPSG:4326")
...
def generating_swatplus_pcp(VPUID, PRISM_VPUID_grid, datasets, years,SWAT_MODEL_PRISM_path ):
    ...
    for row, col in zip(PRISM_VPUID_grid.row, PRISM_VPUID_grid.col):
        ...
        if SWAT_MODEL_PRISM_path is None:
            path_to_save = os.path.join(f"{SWATGenXPaths.PRISM_path}/VPUID/{VPUID}/", f"r{row}_c{col}.pcp")
        else:
...
def generating_swatplus_tmp(VPUID, PRISM_VPUID_grid, datasets_max,datasets_min ,years,SWAT_MODEL_PRISM_path):
    ...
    for row, col in zip(PRISM_VPUID_grid.row, PRISM_VPUID_grid.col):
        ...
        lon = PRISM_VPUID_grid[(PRISM_VPUID_grid.row==row) & (PRISM_VPUID_grid.col==col)].lon.values[0]
        elev = elev_data[row, col]
        path_to_save = f"{SWATGenXPaths.PRISM_path}/VPUID/{VPUID}/r{row}_c{col}.tmp"

        if SWAT_MODEL_PRISM_path is None:
            path_to_save = os.path.join(f"{SWATGenXPaths.PRISM_path}/VPUID/{VPUID}/", f"r{row}_c{col}.tmp")
        else:
...
def extract_PRISM_parallel(VPUID, list_of_huc12s=None, SWAT_MODEL_PRISM_path=None):
    ...
    if list_of_huc12s is not None:
        ...
        _temp_PRISM_grid = gpd.read_file(SWATGenXPaths.PRISM_mesh_path)
    ...
    for YEAR in YEARS:
        path = f"{SWATGenXPaths.PRISM_path}/CONUS/ppt/{YEAR}.nc"
    ...
    start = time.time()
    for YEAR in YEARS:
        path_max = f"{SWATGenXPaths.PRISM_path}/CONUS/tmax/{YEAR}.nc"
        path_min = f"{SWATGenXPaths.PRISM_path}/CONUS/tmin/{YEAR}.nc"
...
if __name__ == "__main__":
    ...
    processes = []
    extract_PRISM_parallel("0407", list_of_huc12s=None, SWAT_MODEL_PRISM_path=f"{SWATGenXPaths.swatgenx_outlet_path}/0407/huc12/04128990/PRISM")