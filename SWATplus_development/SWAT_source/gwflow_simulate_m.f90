      ...
      !flow variables
      integer  i,j,k,n,ob_num,ob_num_aqu,num_hru,row,col,chan_num,chan_gis,num_ts,chan_ob_num
      integer  riv_num,riv_ob_num,count,cell_row,cell_col
      real     thick_west,thick_east,thick_north,thick_south
      real     recharge
      ...
      real     ss_rech,ss_et,ss_gw,ss_sw,ss_satex,ss_tran,ss_pumpag,ss_pumpex,ss_Q,ss_tile, &
               ss_lake,mass_error
      ...
      real     tran_volume,dist_above,hru_area_m2,water_depth_tot,layer_fraction,layer_transfer,sol_thick
      ...
      !no3
      real     recharge_n,rech_nmass,cell_rech_nmass,nmass,chan_cn,nmass_west,nmass_east,nmass_north,nmass_south,satex_nmass
      ...
      !p
      real     recharge_p,rech_pmass,cell_rech_pmass,pmass,chan_cp,pmass_west,pmass_east,pmass_north,pmass_south,satex_pmass
          ...
          do j=1,grid_ncol
            if(gw_cell_status(i,j).gt.0) then !only proceed if the cell is active
              if(gw_cell_status(i,j).eq.1) then !interior cell
                ...
                head_change = (Q_west + Q_east + Q_north + Q_south + gw_cell_ss(i,j)) * &
					(gw_time_step/(gw_cell_Sy(i,j) * cell_size * cell_size))
            ...
            do i=1,grid_nrow
              do j=1,grid_ncol
                if(gw_cell_status(i,j).eq.1) then !interior cell
                  ...
                  mn_change = (nmass_adv(i,j) + nmass_dsp(i,j) + nmass_rct(i,j) + gw_cell_ssn(i,j)) * &
				  	(gw_trans_time_step / gw_reta_no3)
                  mp_change = (pmass_adv(i,j) + pmass_dsp(i,j) +                  gw_cell_ssp(i,j)) * & 
				  	(gw_trans_time_step / gw_reta_p)
...
107   format(i8,i8,f10.2,2x,e13.6,2x,e13.6,50(f12.2))
108   format(i8,2x,50(e12.4))
109   format(i8,i8,1000(e12.3))

      return
      end subroutine gwflow_simulate_m