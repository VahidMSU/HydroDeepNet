      ...
      use hru_module, only : hru, cn2, fertno3, fertnh3, fertorgn, fertorgp, fertsolp,   &
        ihru, ipl, isol, phubase, sol_sumno3, sol_sumsolp ! ndeat,
      ...
      implicit none
      external pl_fert, pl_fert_wet ,mgt_transplant ,mgt_harvtuber ,mgt_harvbiomass ,mgt_harvgrain ,mgt_killop
      external pest_apply,pfas_apply ,pl_graze,wet_initial ,mgt_newtillmix_wet,mgt_newtillmix ,hru_lum_init,plant_init 
      external cn2_init,structure_set_parms,pl_burnop ,curno, chg_par, hru_fr_change
              ...
              do ipl = 1, pcom(j)%npl
                ...
                if (d_tbl%act(iac)%option == pcomdb(icom)%pl(ipl)%cpnm) then
                  ...
                  if (pcom(j)%plcur(ipl)%gro == "n") then
                    ...
                    if (allocated(d_tbl%act_app)) then
                      if (d_tbl%act_app(iac) > 1000) then
                        write(9001,*) "Error: Invalid value for d_tbl%act_app.", "ihru", ihru, "iac", iac, "d_tbl%act_app(iac)", d_tbl%act_app(iac)

                      end if
                    else
                      write(9001,*) "Error: d_tbl%act_app is not allocated."
                    end if
