...
!      end if
      ...
      if (sp_ob%gwflow > 0) then
        ...
        write(9001,*) 'Completed reading gwflow.con file'
        write(9001,*) 'Number of gwflow objects: ', sp_ob%gwflow
        write(9001,*) 'First gwflow object number: ', sp_ob1%gwflow
        ! Example of printing out some attributes of the first gwflow object
        ! (Adjust the index as necessary to print specific or multiple objects)
        if (sp_ob%gwflow > 0) then
            write(9001,*) 'Details of first gwflow object:'
            write(9001,*) 'Object Number: ', ob(sp_ob1%gwflow)%num
            write(9001,*) 'Name: ', ob(sp_ob1%gwflow)%name
            write(9001,*) 'GIS ID: ', ob(sp_ob1%gwflow)%gis_id
            write(9001,*) 'Area (ha): ', ob(sp_ob1%gwflow)%area_ha
            write(9001,*) 'Cell ID: ', ob(sp_ob1%gwflow)%props
            write(9001,*) 'Channels: ', ob(sp_ob1%gwflow)%obtypno_out
      ...
      do i = 1, sp_ob%objs
          do ii = 1, ob(i)%src_tot
            ...
            case ("sdc")   !swat-deg channel
              ...
              j = ob(i)%obj_out(ii)
              ob(j)%rcv_tot = ob(j)%rcv_tot + 1   !!!! this is the line that the model crash. 
            end select
            ! Proceed with operations using j as an index
            select case (ob(i)%htyp_out(ii))  !! this is line that failes
            case ("tot")   !total flow
               ob(i)%ihtyp_out(ii) = 1
            case ("rhg")   !recharge
               ob(i)%ihtyp_out(ii) = 2              
