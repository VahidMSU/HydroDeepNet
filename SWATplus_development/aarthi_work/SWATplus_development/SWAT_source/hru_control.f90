...
!!    hydrologic cycle
      ...
      sw_volume_begin = sum(soil(j)%phys(:)%st)
      ...
      if (nd_30 > 30) nd_30 = 1
        ...
        if (sched(isched)%num_autos > 0) then
          do iauto = 1, sched(isched)%num_autos
            ...
            jj = j
            !write(9001,*) 'id', id, 'iauto', iauto, 'dtbl_lum(id)', dtbl_lum(id)%acts
            !if (.not. allocated(dtbl_lum)) then
            !  print *, "dtbl_lum is not allocated"
            !end if
            !if (size(dtbl_lum) < 1) then
            !  print *, "dtbl_lum does not have an element at index 1"
            !end if
            d_tbl => dtbl_lum(id)   !!! line of error
            call conditions (jj, iauto)
            call actions (jj, iob, iauto)
        ...
        if (in_snodas%flag == 1) then
          ...
          if (time%yrc > 2003 .and. time%yrc <2023) then  !! Modified condition
          ! write(9001,*) 'call sq_snom_snodas', time%yrc, time%day, "in_snodas%flag", in_snodas%flag
            call sq_snom_snodas
          else
            call sq_snom
          end if
